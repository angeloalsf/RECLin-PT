...
the two baselines are statistically tied on the \texttt{negation\_of} F1 under the
reference seed (0.724 vs.\ 0.734; McNemar $p=0.18$; paired bootstrap 95\% CI
$[-0.047,+0.026]$), but this verdict does not hold across the two seeds we tested:
seed-to-seed variation exceeds the gap between the models, and the clinical
encoder leads under neither seed. These initial results
...
estatisticamente no F1 de \texttt{negation\_of} na semente de referência (0,724
vs.\ 0,734; McNemar $p=0,18$; \emph{bootstrap} pareado IC95\% $[-0,047;+0,026]$),
mas esse veredito não se sustenta nas duas sementes testadas: a variação entre
sementes supera a diferença entre os modelos, e o \emph{encoder} clínico não
lidera em nenhuma delas. Esses resultados iniciais indicam que o
...
\emph{encoder} de domínio geral alcança o clínico --- e, nas sementes testadas,
nunca fica atrás dele --- inclusive na classe de negação, com cerca de 60\% dos
parâmetros. O restante do trabalho está
...
representação da classe minoritária em todas elas. A partição é gerada com
semente fixa (42) e congelada. O resultado são 800 documentos de treino, 100 de
desenvolvimento e 100 de teste, com 14,09\%, 15,70\% e 12,16\% de
\texttt{negation\_of}, respectivamente. A estratificação atua sobre a
\emph{presença} das classes em cada documento, e não sobre a contagem de
relações: ela garante que nenhuma partição fique sem exemplos da classe rara, mas
não iguala percentuais --- daí a variação de cerca de três pontos percentuais. O
que importa para o protocolo é a garantia: as 152 relações \texttt{negation\_of}
do teste tornam a estimativa de F1 dessa classe interpretável, ainda que com
incerteza amostral relevante --- razão pela qual toda comparação é acompanhada de
teste pareado e intervalo de confiança, e não apenas do valor pontual.
...
\subsection{Representação de entrada: marcadores de posição}
Cada par candidato é transformado na \emph{string} de entrada do \emph{encoder}
por uma janela de $\pm128$ caracteres em torno do par, com as duas entidades
delimitadas por marcadores, no esquema
\texttt{[E1] superfície [/E1] $\ldots$ [E2] superfície [/E2]} --- técnica que se
tornou padrão para RE com modelos baseados em BERT~\cite{soares:19}. Os quatro
marcadores são registrados como \emph{tokens} especiais no \emph{tokenizer}, com
\emph{embedding} dedicado aprendido no ajuste fino.

Os marcadores são \emph{posicionais}, não semânticos: indicam quais trechos da
janela formam o par e qual deles é o primeiro argumento, e nada mais. O tipo
semântico anotado pelo SemClinBr (\texttt{Disease or Syndrome},
\texttt{Negation}, \texttt{Finding}\ldots) \emph{não} é passado ao modelo, que
deve inferir a natureza de cada entidade da superfície textual e do contexto.
Como a inserção é feita por deslocamento de caractere, a direção da relação ---
que importa para \texttt{negation\_of} --- é preservada por \emph{qual} marcador
envolve \emph{qual} entidade, e não pela ordem em que aparecem na \emph{string}.
A classificação usa a representação agregada da sequência, e não uma extração
explícita dos estados ocultos dos marcadores: estes influenciam a predição por
meio da atenção, ao condicionar a codificação da janela inteira ao par
delimitado. Como a mesma representação de entrada é aplicada aos dois
\emph{baselines}, qualquer efeito dessas escolhas é compartilhado e não confunde
a comparação --- e a ausência do tipo é, para a pergunta em foco, conveniente:
como nenhum dos dois \emph{encoders} recebe a informação semântica pronta, a
comparação mede a capacidade de cada um de inferi-la do texto.
...
marcadores de posição; a perda de entropia cruzada com
...
robusta. Como análise de robustez à variação de inicialização, ambos os
\emph{baselines} são treinados também com uma segunda semente (43).
...
% Tabela gerada por scripts/make_tables.py a partir de results/*.json.
% O fragmento traz \caption e \label{tab:resultados}. Nao edite a mao:
% para atualizar, rode `python scripts/make_tables.py`.
\input{tables/tab_resultados.tex}
...
\emph{nesta semente} --- ressalva relevante, pois a análise de robustez à semente,
a seguir, mostra que esse veredito não se repete na semente 43.

% Tabela gerada por scripts/make_tables.py a partir de results/*.json.
% O fragmento traz \caption e \label{tab:signif}. Nao edite a mao:
% para atualizar, rode `python scripts/make_tables.py`.
\input{tables/tab_signif.tex}

\subsection{Robustez à semente}
Para situar a magnitude da diferença entre os \emph{baselines}, treinamos ambos
com uma segunda semente (43). No BioBERTpt, o macro-F1 cai de 0,707 (semente 42)
para 0,665 (semente 43), e o F1 de \texttt{negation\_of}, de 0,724 para 0,653 ---
uma variação de 0,071 atribuível \emph{apenas} à inicialização aleatória do
treino. Essa variação entre sementes de um \emph{mesmo} modelo é cerca de sete
vezes maior que a diferença de 0,010 observada entre o BioBERTpt e o BERTimbau na
...
O BERTimbau, sob a mesma troca de semente, mostra-se sensivelmente mais estável:
o macro-F1 passa de 0,704 para 0,706 e o F1 de \texttt{negation\_of}, de 0,734
para 0,726 --- uma amplitude de 0,008, cerca de nove vezes menor que a do
BioBERTpt na mesma métrica. Na semente 43 a ordenação entre os dois
\emph{baselines} no F1 de negação se mantém, com margem de 0,073, e o mesmo
protocolo de significância aplicado à semente 42 acusa agora uma diferença
significativa em favor do BERTimbau (McNemar $p<10^{-30}$; \emph{bootstrap}
pareado IC95\% $[-0,115;\,-0,031]$, $p=0,0002$) --- ao contrário do que se observa
na semente 42, em que o mesmo teste não rejeita a hipótese nula. Esse contraste
não estabelece a superioridade do \emph{encoder} geral: o que ele mostra é que o
próprio \emph{veredito} do teste pareado, sob protocolo e conjunto de teste
idênticos, alterna entre ``sem diferença'' e ``diferença significativa'' em função
apenas da inicialização do treino. O que atravessa as duas sementes é o sinal ---
em nenhuma delas o pré-treinamento clínico aparece à frente --- e a menor
previsibilidade do \emph{encoder} clínico entre reinicializações, o que, em um
cenário de implantação, pesa a favor do geral.

\subsection{Análise dos resultados}
O resultado central é a \emph{ausência de vantagem do pré-treinamento clínico}:
para a extração de relações no SemClinBr, no espaço de três classes, o BioBERTpt
não produz ganho mensurável sobre o BERTimbau, nem mesmo na classe de negação,
onde a especialização de domínio seria mais plausível. Convém precisar o que os
testes sustentam: o veredito de empate vale para a semente 42 e não se repete na
semente 43, em que a comparação pareada favorece o BERTimbau. O que se mantém
entre as duas sementes não é o empate, e sim a direção --- em nenhuma delas o
\emph{encoder} clínico fica à frente. Esse achado contrasta com a expectativa
difundida --- e com os ganhos
...
tão bem quanto um clínico --- ainda mais porque o gatilho de negação está
tipicamente adjacente ao achado negado. Segundo, a tarefa, tal
como formulada, depende mais da estrutura local do par (direção, proximidade,
delimitação das entidades) do que de conhecimento lexical especializado de
...
ele não afeta a comparação, mas limita o desempenho absoluto reportado. Uma
ressalva mais séria recai sobre a própria caracterização de empate: ela vale para
a semente 42, e a comparação pareada refeita na semente 43 acusa diferença
...
estatística, porém, depende, e não deve ser tomada como definitiva a partir de uma
única execução por modelo. Duas sementes por modelo tampouco bastam para estimar
um intervalo de confiança sobre essa variação; fechar a questão exige um número
maior de repetições com agregação formal.
...
idêntica que varia apenas o \emph{checkpoint} de pré-treino, não se observou
vantagem do pré-treinamento clínico. Na semente 42, a diferença no F1 de
\texttt{negation\_of} é de 0,010 e não é significativa (McNemar $p=0,18$;
\emph{bootstrap} pareado IC95\% $[-0,047;+0,026]$); na semente 43, a mesma
comparação pareada passa a favorecer o BERTimbau. O veredito estatístico,
portanto, não é estável entre sementes --- a variação de inicialização de um mesmo
modelo supera com folga a diferença entre eles ---, mas a direção é: em nenhuma
das sementes testadas o \emph{encoder} clínico fica à frente. O geral alcança o
clínico com cerca de 60\% dos parâmetros.
...
\emph{baselines}. Como o BERTimbau geral não fica atrás em nenhuma das sementes
testadas na métrica-alvo \texttt{negation\_of} e exige cerca de 60\% dos
parâmetros, ele se apresenta como a base mais atraente por sua eficiência. O
...
treino, para um intervalo de confiança da diferença mais estreito; enriquecer a
representação de entrada com o tipo semântico das entidades, hoje ausente dos
marcadores, medindo se o ganho difere entre o \emph{encoder} clínico e o geral;
estender o
