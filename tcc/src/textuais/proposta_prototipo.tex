...
\begin{itemize}
  ...
  \item o mesmo critério de seleção de modelo --- maior macro-F1 no conjunto de
        validação --- e a mesma semente de treino.
\end{itemize}
...
ser atribuída ao pré-treinamento, e não a artefatos de implementação --- que é
...
\foreignlanguage{english}{BERT}: inserir no próprio texto marcadores que delimitam
$e_1$ e $e_2$ é mais eficaz do que concatenar representações de \emph{span}
isoladas, pois permite que o mecanismo de atenção condicione a codificação de toda
a janela ao par sob análise.
...
\begin{enumerate}
  \item \textbf{Os marcadores são posicionais, não semânticos.} Eles indicam
        quais trechos da janela formam o par e qual deles é o primeiro argumento
        --- e nada mais. O \textbf{tipo semântico} anotado pelo SemClinBr
  ...
  \item \textbf{A direção vem da etiqueta, não da ordem.} A inserção é feita por
        ...
        \emph{string} resultante começa por \texttt{[E2]}. A direção da relação
        --- que importa para \texttt{negation\_of} --- é preservada por
...
comparação mede a capacidade de cada um de \emph{inferi-la} do texto --- que é
exatamente onde o pré-treinamento de domínio deveria fazer diferença.
