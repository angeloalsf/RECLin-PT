...
filtro de pista lexical\footnote{Pista lexical é uma palavra ou expressão cuja
simples presença no texto sinaliza um fenômeno linguístico, aqui a negação. O
filtro verifica apenas se o primeiro argumento de cada par predito como
\texttt{negation\_of} é uma dessas pistas, sem analisar o restante da sentença.} calibrado no conjunto de desenvolvimento.
...
Apêndice~\ref{ap:lexico}). O léxico de gatilhos\footnote{Neste método, gatilho é
cada forma retida no léxico induzido do treino, e um par começa por um gatilho
quando o texto de $e_1$, após a normalização descrita adiante, coincide com uma
dessas formas. O conceito é operacional e distinto dos marcadores do
Apêndice~\ref{ap:lexico}, que caracterizam o fenômeno linguisticamente e não
participam do filtro.} empregado pelo filtro
...
cuja frequência atinge o limiar \texttt{min\_freq}\footnote{\texttt{min\_freq} é o número
mínimo de pares candidatos \texttt{negation\_of} do treino em que uma forma
precisa figurar como $e_1$ para ser retida no léxico, e o seu valor é calibrado
na Seção~\ref{sec:calibracao}.}.

As formas são comparadas sob normalização por decomposição NFKD\footnote{NFKD
(\foreignlanguage{english}{Normalization Form Compatibility Decomposition}) é uma
das formas de normalização do padrão Unicode, que decompõe cada letra acentuada
em letra-base seguida de um diacrítico combinante, o que permite remover o
acento descartando este último.}, remoção de
diacríticos, colapso de espaço em branco e \emph{casefold}\footnote{\emph{Casefold} é a conversão para
caixa baixa definida pelo padrão Unicode para comparar textos sem distinguir
maiúsculas de minúsculas, de modo que ``NEGA'' e ``nega'' passam a ter a mesma
forma.}. A remoção de
...
\emph{recall}, apenas a precisão, e a cobertura do léxico\footnote{Cobertura do léxico é a
proporção, entre os pares candidatos de uma partição cujo rótulo de referência é
\texttt{negation\_of}, daqueles cujo $e_1$ normalizado pertence ao léxico.} passa a ser um teto de
